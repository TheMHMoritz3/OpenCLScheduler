 ...
 */
__kernel void temp(global const unsigned float* input, global const float* min, global const float*max, global const int* result, const int wLoad) {
    int gid = get_global_id(0) * wLoad;

    for (int i = 0; i < wLoad; ++i){
        unsigned int in = input[gid + i];

        result[gid+i] = inRange(input[gid+i], min[0], max[0]);
    }
} 

//Mit Array Min,Max
__kernel void temp1(global const unsigned float* input, global const float* min, global const float*max, global const int* result, const int wLoad) {
    int gid = get_global_id(0) * wLoad;
