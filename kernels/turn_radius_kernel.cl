 ...
 */
__kernel void radius(global const float* axleLength, global const float* speedL, global const float* speedR, global float* result, const int WLOAD){
    int gid = get_global_id(0) * WLOAD;
    

    for (int i = 0; i < WLOAD; ++i){
        const float axleSize = axleLength[gid + i];
