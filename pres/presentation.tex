...
\begin{frame}{Example Raspberry Pi 3 B+}
...
\begin{frame}{Mesurements}
	\begin{columns}
        ...
        Testing procedure:
		\begin{enumerate}
			\item Running Tasks multible times with the same settings
			\item Calculating the average of all runs (yellow line)
			\item Presenting the collected data in a reasonable manner 
		\end{enumerate}
	\end{columns}
\end{frame}
\subsection{Results}
\begin{frame}
   \frametitle{Raspberrry PI 3 B - POCL (CPU)}
   \includegraphics[width=.8\textwidth]{res/ResultsPOCL.PNG}\\
   Compared to serial schedulers, parallel schedulers are much faster.
\end{frame}
\begin{frame}
	\frametitle{Raspberrry PI 3 B - POCL - Without Serial}
	\includegraphics[width=.8\textwidth]{res/ResultsPOCL_WithoutSerial.PNG}
\end{frame}
\begin{frame}
	\frametitle{Raspberrry PI 3 B - POCL - 100k}
...
\end{frame}
\begin{frame}
	\frametitle{Raspberrry PI 3 B - VC4CL (GPU)}
	\includegraphics[width=.8\textwidth]{res/ResultsVC4CL.PNG}\\
    Again: Compared to serial schedulers, parallel schedulers are much faster.
\end{frame}
\begin{frame}
	\frametitle{Raspberrry PI 3 B - VC4CL - Without Serial}
	\includegraphics[width=.8\textwidth]{res/ResultsVC4CL_WithoutSerial.PNG}
\end{frame}
\begin{frame}
	\frametitle{Raspberrry PI 3 B - VC4CL - 20k}
...
\begin{frame}
	...
	\begin{itemize}
		\item On POCL ASAP and Static schedulers have similar execution times. Only minor diffrences caused by algorithmic overhead can be detected.
		\item The most efficent scheduler on VC4CL is the static scheduler. 
		\item The effects of other tasks on POCL are highly negative (e.g. moving the mouse).
		\item OpenCLs ``Out of Order``-flag has only minor but positive effects.
	\end{itemize}
\end{frame}
\begin{frame}
	\frametitle{Conclusion}
	\begin{itemize}
		\item Workgroup sizes on POCL have little to no impact.
		\item In our case CPU and GPU performance can be close.\\
            $\Rightarrow$ This could be caused by caching problems and memory latency on the GPU.
        \item Workgroup sizes on both CPU and GPU should always be maxed out.
        \item More than 5-10 million datapoints per OpenCL-kernel can easily lead to memory overflow.\\
            $\Rightarrow$ Memory management optimization would be required.
	\end{itemize}
\end{frame}
\end{document}
